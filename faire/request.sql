...
faire_dbr AS (
    SELECT
        ...
        COALESCE(g.gross_rev, 0) AS faire_gross_rev,
        COALESCE(g.discounts, 0) AS faire_discounts,
        COALESCE(g.shipping, 0) AS faire_shipping,
        COALESCE(r.returns, 0) AS faire_returns,
        faire_gross_rev
            + faire_discounts
            + faire_shipping
            + faire_returns AS faire_net_rev,
        COUNT(DISTINCT g.order_id) AS faire_orders,
        COALESCE(g.units_ordered, 0) AS faire_units_ordered,
        COALESCE(r.units_returned, 0) AS faire_units_returned,
        faire_units_ordered
